                ...
                <form onSubmit={onSubmit} className="space-y-4">
                    ...
                    <WsField label="Ảnh bìa bài viết">
                        <Pattern
                            ...
                            onUploadComplete={(urls) => onChange({ ...form, coverImage: urls[0] || "" })}
                    ...
                    <div className="flex items-center justify-between py-2 border-t border-b border-border/40">
                        <span className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">Xuất bản bài viết</span>
                        <Switch
                            checked={form.published}
                            onCheckedChange={(checked) => onChange({ ...form, published: checked })}
                        />
                    </div>

                    <div className="space-y-1">
                        <span className="text-[0.6rem] tracking-widest uppercase text-muted-foreground block font-semibold mb-1">Nội dung bài viết (CKEditor)</span>
                        <CKEditor
                            value={form.content}
                            ...
                            cells: [
                                <div key="title">
                                    ...
                                    <div className="flex items-center gap-1.5 mt-0.5">
                                        <Badge variant={item.published ? "success" : "secondary"} size="sm">
                                            {item.published ? "Đã đăng" : "Bản nháp"}
                                        </Badge>
                                    </div>
                                </div>,
                                <div key="meta">
                                    <p className="text-xs font-semibold">{item.author || "Chiến Nguyễn"}</p>
                                    <p className="text-xs text-muted-foreground">{item.category || "General"}</p>
                                </div>,
                                <span key="date" className="text-xs text-muted-foreground font-mono">
                  {formattedDate}
                </span>,
                            ],
                            onEdit: () => onEdit(item),
